...
class GameScene: SKScene {
    ...
    func restartGame() {
        ...
        chainDelay = 3.0
        sequencePosition = 0
        nextSequenceQueued = false   // 🔹 FIXED (was true, so enemies didn’t spawn again)
        activeEnemies.removeAll()
        
        // Reset lives images
        for (i, life) in livesImages.enumerated() {
            life.texture = SKTexture(imageNamed: "sliceLife")
            life.position = CGPoint(x: CGFloat(834 + (i * 70)), y: 700)
        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard let touch = touches.first else { return }
        let locations = touch.location(in: self)
        
        let tappedNodes = nodes(at: locations)
